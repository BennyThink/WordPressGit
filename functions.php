...
{
    if (!is_single() || get_post_type() == 'gallery' || get_post_type() == 'product') {
        return false;
    }
...
function footerScript()
{
    if (!is_admin()) {
        ...
        if (git_get_option('git_jqcdn') == 'git_jqcdn_qiniu') {
            wp_register_script('jquery', '//cdnjs.cloudflare.com/ajax/libs/jquery/1.8.3/jquery.min.js', false, '1.0', false);
...
function deel_share()
{
    if (!git_get_option('git_bdshare_b')) {
        return false;
    }
...
';
        echo "<script>new needShareButton(document.getElementById('i-share'));</script>";
...
function timeago($ptime)
{
    ...
    $interval = array(
        ...
        24 * 60 * 60 => '天前(' . date('m-d H:i', $ptime) . ')',
        60 * 60 => '小时前',
        60 => '分钟前',
        1 => '秒前'
    );
    ...
    if (empty($result)) {
        ...
        foreach ($result as $topten) {
            ...
            if ($commentcount != 0) {
                ...
                if (isset($_COOKIE['bigfa_ding_' . $postid])) {
                    $output .= ' actived';
                }
...
function git_copyright($content)
{
    if (is_single() || is_feed()) {
        $copyright = str_replace(array('{{title}}', '{{link}}'), array(
            get_the_title(),
            get_permalink()
...
function zfunc_smiley_button($custom = false, $before = '', $after = '')
{
    if ($custom == true) {
        ...
        $customsmiley_url = get_template_directory_uri() . '/css/img/smilies';
    }
    echo $before;
    ?>
...
{
    if (get_post_meta($post_ID, 'git_weibo_sync', true) == 1) {
        return;
    }
    ...
    public function edit_user_profile_update($user_id)
    {
        ...
        if (!empty($_FILES['simple-local-avatar']['name'])) {
            ...
            if (!function_exists('wp_handle_upload')) {
                require_once(ABSPATH . 'wp-inc/includes/file.php');
            }
            ...
            if (empty($avatar['file'])) { // handle failures
                switch ($avatar['error']) {
                    case 'File type does not meet security guidelines. Try another.':
                        add_action('user_profile_update_errors', function ($a) {
                            $a->add("avatar_error", __("请上传有效的图片文件。", "simple-local-avatars"));
                        });
                        break;
...
if (git_get_option('git_ua_b')):
    ...
    function user_agent($ua)
    {
        ...
        $prePath1 = get_template_directory_uri() . '/css/img/ua/';
        $prePath2 = $prePath1;
        //$test1=$prePath1;
        //$test2=$prePath2;
        //确定UA图片
        if (strstr($os, 'Vista')) {
            $prePath1 .= 'Vista' . '.png';
        } elseif (strstr($os, 'Windows 7')) {
            $prePath1 .= 'Windows7' . '.png';
        } elseif (strstr($os, 'Windows 8')) {
            $prePath1 .= 'Windows8' . '.png';
        } elseif (strstr($os, 'Windows 8.1')) {
            $prePath1 .= 'Windows8.1' . '.png';
        } elseif (strstr($os, 'Windows 10')) {
            $prePath1 .= 'Windows10' . '.png';
        } elseif (strstr($os, 'Windows XP')) {
            $prePath1 .= 'WindowsXP' . '.png';
        } elseif (strstr($os, 'Windows 2000')) {
            $prePath1 .= 'Windows2000' . '.png';
        } elseif (strstr($os, 'Android')) {
            $prePath1 .= 'Android' . '.png';
        } elseif (strstr($os, 'iPhone')) {
            $prePath1 .= 'ios' . '.png';
        } elseif (strstr($os, 'iPad')) {
            $prePath1 .= 'ios' . '.png';
        } elseif (strstr($os, 'Mac')) {
            $prePath1 .= 'Mac' . '.png';
        } elseif (strstr($os, 'Windows Phone')) {
            $prePath1 .= 'WindowsPhone' . '.png';
        } elseif (strstr($os, 'Gentoo')) {
            $prePath1 .= 'Gentoo' . '.png';
        } elseif (strstr($os, 'Ubuntu')) {
            $prePath1 .= 'Ubuntu' . '.png';
        } elseif (strstr($os, 'Debian')) {
            $prePath1 .= 'Debian' . '.png';
        } elseif (strstr($os, 'FreeBSD')) {
            $prePath1 .= 'FreeBSD' . '.png';
        } elseif (strstr($os, 'SunOS') || strstr($os, 'Solaris')) {
            $prePath1 .= 'Sun' . '.png';
        } elseif (strstr($os, 'BlackBerry')) {
            $prePath1 .= 'BlackBerry' . '.png';
        } elseif (strstr($os, 'Linux')) {
            $prePath1 .= 'Linux' . '.png';
        } else {
            $prePath1 .= 'unknowOS' . '.png';
        }
        //浏览器图片
        if (strstr($browser, 'Camino')) {
            $prePath2 .= 'Camino' . '.png';
        } elseif (strstr($browser, '搜狗浏览器')) {
            $prePath2 .= 'sogou' . '.png';
        } elseif (strstr($browser, '360浏览器')) {
            $prePath2 .= '360' . '.png';
        } elseif (strstr($browser, 'Maxthon')) {
            $prePath2 .= 'Maxthon' . '.png';
        } elseif (strstr($browser, 'Edge')) {
            $prePath2 .= 'Edge' . '.png';
        } elseif (strstr($browser, '微信')) {
            $prePath2 .= 'weixin' . '.png';
        } elseif (strstr($browser, 'QQ')) {
            $prePath2 .= 'QQ' . '.png';
        } elseif (strstr($browser, 'Chrome')) {
            $prePath2 .= 'Chrome' . '.png';
        } elseif (strstr($browser, 'Chromium')) {
            $prePath2 .= 'Chromium' . '.png';
        } elseif (strstr($browser, '小米')) {
            $prePath2 .= 'xiaomi' . '.png';
        } elseif (strstr($browser, 'Safari')) {
            $prePath2 .= 'Safari' . '.png';
        } elseif (strstr($browser, 'Opera')) {
            $prePath2 .= 'Opera' . '.png';
        } elseif (strstr($browser, '腾讯TT浏览器')) {
            $prePath2 .= 'tt' . '.png';
        } elseif (strstr($browser, 'QQ浏览器')) {
            $prePath2 .= 'qqbrowser' . '.png';
        } elseif (strstr($browser, 'UCWEB')) {
            $prePath2 .= 'ucweb' . '.png';
        } elseif (strstr($browser, 'Internet Explorer')) {
            $prePath2 .= 'ie' . '.png';
        } elseif (strstr($browser, 'WordPress客户端')) {
            $prePath2 .= 'wordpress' . '.png';
        } elseif (strstr($browser, 'Firefox')) {
            $prePath2 .= 'firefox' . '.png';
        } else {
            $prePath2 .= 'unknowBrowser' . '.png';
        }
        //end
...
if (!function_exists('Baidu_Submit') && git_get_option('git_sitemap_api')) {
    function Baidu_Submit($post_ID)
    {
        ...
        $request = new WP_Http;
        $result = $request->request($apiurl, array(
            'method' => 'POST',
            'body' => $url,
    ...
    if (!empty($linkcats)) {
        foreach ($linkcats as $linkcat) {
            ...
            if ($linkcat->description) {
                $result .= '<div class="link_description">' . $linkcat->description . '</div>';
            }
...
function send_new_post($new_status, $old_status, $post)
{
    if ('publish' === $new_status && 'publish' !== $old_status && $post->post_type === 'post') {
        ...
        $message = "新的博文《 $post->post_title 》发布了\n$link";

        if (git_get_option('git_tg_uid') && git_get_option('git_tg_bot_token')) {
            $postdata = http_build_query(array(
                'chat_id' => git_get_option('git_tg_uid'),
                'parse_mode' => "html",
                "text" => $message
            ));
            $opts = array(
            ...
            $token = git_get_option('git_tg_bot_token');
